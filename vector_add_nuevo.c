...
//Creando vectores random
//funciona como el Read_vector
void Random_vector(double a[], int n, char vec_name[]);
//incluidos
void Read_n(int* n_p);
...
//inicializamos los vectores aleatorios
   Random_vector(x, n, "se genero x");
   Random_vector(y, n, "se genero y");

//para cada vector
   Print_vector(x, n, "x es igual a: ");
   Print_vector(y, n, "y es igual a: ");

//aqui comienza a tomar el tiempo para que sea objetivo
   clock_t empieza = clock();
//realizamos la suma
   Vector_sum(x, y, z, n);
//acabamos el tiempo
   clock_t finaliza = clock();

   Print_vector(z, n, "The sum is");

//tiempos
   tiempo += (double)(finaliza - empieza) / CLOCKS_PER_SEC;
   printf("Tomo %f secs\n", tiempo);
...
* Porpose: crear dos vectores de al menos 100,000 elementos generados de forma aleatoria.
* Purpose 2:   Read a vector from stdin
